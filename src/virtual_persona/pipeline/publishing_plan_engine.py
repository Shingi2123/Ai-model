    ...
    def generate(self, package: DailyPackage) -> List[PublishingPlanItem]:
        ...
        for idx, ranked_moment in enumerate(selected):
            ...
            item = PublishingPlanItem(
                ...
                action_family=str(getattr(behavior, "action_family", "")),
                emotional_tone_family=str(getattr(daily_behavior, "emotional_tone_family", "") if daily_behavior else ""),
                social_context_hint=str(getattr(behavior, "social_context_hint", "")),
                social_presence_detail=str(getattr(behavior, "social_presence_detail", "")),
                caption_voice_constraints=", ".join(getattr(behavior, "caption_voice_constraints", []) or []),
